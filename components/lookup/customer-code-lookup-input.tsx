...
import { useCustomerCodeLookup } from "@/hooks/use-lookup"
...
export interface CustomerCodeLookupInputProps<
...
> {
  ...
  isRequired?: boolean
  isDisabled?: boolean
  onSelect?: (customerCode: string, customerName: string) => void
}

export default function CustomerCodeLookupInput<
  T extends Record<string, unknown>,
>({
  form,
  name,
  ...
  isDisabled = false,
  onSelect,
}: CustomerCodeLookupInputProps<T>) {
  ...
  const { data: options = [], isLoading } =
    useCustomerCodeLookup(debouncedSearch)

  const list = useMemo(
    () =>
      (options as ICustomerLookup[]).filter(
        (c) => c && (c.customerCode != null || c.customerId != null)
      ) as ICustomerLookup[],
    [options]
  )
  ...
  // Sync form value to input when form changes externally (e.g. reset)
  const formValue = form.watch(name)
  useEffect(() => {
    const v = formValue != null ? String(formValue) : ""
    setInputValue(v)
  }, [formValue, name])
        ...
        createPortal(
          ...
          >
            {isLoading ? (
              ...
              <ul className="p-1">
                {list.map((item, i) => (
                  <li key={item.customerId ?? item.customerCode ?? i}>
                    <button
                      type="button"
                      className={cn(
