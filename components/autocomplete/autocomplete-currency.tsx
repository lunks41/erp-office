...
interface CurrencyAutocompleteProps<T extends Record<string, unknown>> {
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICurrencyLookup | null) => void
}
...
>({
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCurrency = selectedOption
          ? currencys.find(
              (u: ICurrencyLookup) =>
                u.currencyId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCurrency)
      }
    },
    [form, name, onChangeEvent, currencys]
  )
