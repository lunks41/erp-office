...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IAccountGroupLookup | null) => void
}) {
  ...
  const options: FieldOption[] = React.useMemo(
    () =>
      accountgroups.map((accountgroup: IAccountGroupLookup) => ({
        ...
        label: accountgroup.accGroupName,
      })),
    [accountgroups]
  )
  ...
  const handleChange = React.useCallback(
    ...
    [form, name, onChangeEvent, accountgroups]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading accountgroups..."}
