...
import { IUserRoleLookup } from "@/interfaces/lookup"
...
import { useUserRoleLookup } from "@/hooks/use-lookup"
...
export default function UserRoleAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IUserRoleLookup | null) => void
}) {
  const { data: userRoles = [], isLoading } = useUserRoleLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      userRoles.map((userRole: IUserRoleLookup) => ({
        value: userRole.userRoleId.toString(),
        label: userRole.userRoleName,
      })),
    [userRoles]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedUserRole = selectedOption
          ? userRoles.find(
              (u: IUserRoleLookup) =>
                u.userRoleId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedUserRole)
      }
    },
    [form, name, onChangeEvent, userRoles, markOptionSelected]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading userRoles..."}
