...
export default function CompanyAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICompanyLookup | null) => void
}) {
  const { data: countries = [], isLoading } = useCompanyLookup()
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCompany = selectedOption
          ? countries.find(
              (u: ICompanyLookup) =>
                u.companyId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCompany)
      }
    },
