...
import { IAccountTypeLookup } from "@/interfaces/lookup"
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IAccountTypeLookup | null) => void
}) {
  const { data: accounttypes = [], isLoading } = useAccountTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      accounttypes.map((accounttype: IAccountTypeLookup) => ({
        value: accounttype.accTypeId.toString(),
        label: accounttype.accTypeName,
      })),
    [accounttypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedAccountType = selectedOption
          ? accounttypes.find(
              (u: IAccountTypeLookup) =>
                u.accTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedAccountType)
      }
    },
    [form, name, onChangeEvent, accounttypes, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading accounttypes..."}
