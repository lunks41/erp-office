...
import { IGstCategoryLookup } from "@/interfaces/lookup"
...
export default function GstCategoryAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IGstCategoryLookup | null) => void
}) {
  const { data: gstCategorys = [], isLoading } = useGstCategoryLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      gstCategorys.map((gstCategory: IGstCategoryLookup) => ({
        value: gstCategory.gstCategoryId.toString(),
        label: gstCategory.gstCategoryName,
      })),
    [gstCategorys]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedGstCategory = selectedOption
          ? gstCategorys.find(
              (u: IGstCategoryLookup) =>
                u.gstCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedGstCategory)
      }
    },
    [form, name, onChangeEvent, gstCategorys, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading gstCategorys..."}
