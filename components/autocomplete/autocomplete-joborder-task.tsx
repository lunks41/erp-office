...
import { ITaskLookup } from "@/interfaces/lookup"
import {
  IconCheck,
  IconChevronDown,
  IconRefresh,
...
import { useJobOrderTaskLookup } from "@/hooks/use-lookup"
...
export default function JobOrderTaskAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ITaskLookup | null) => void
}) {
  const {
    data: tasks = [],
    isLoading,
    refetch,
  } = useJobOrderTaskLookup(jobOrderId)
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedTask = selectedOption
          ? tasks.find(
              (u: ITaskLookup) => u.taskId.toString() === selectedOption.value
            ) || null
          : null
