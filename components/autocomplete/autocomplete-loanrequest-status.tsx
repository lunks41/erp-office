...
import { IStatusTypeLookup } from "@/interfaces/lookup"
...
import { useLoanRequestStatusLookup } from "@/hooks/use-lookup"
import { FormField, FormItem } from "@/components/ui/form"
import { Label } from "@/components/ui/label"

type FieldOption = SearchableFieldOption

export default function LoanRequestStatusAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IStatusTypeLookup | null) => void
}) {
  const { data: statusTypes = [], isLoading } = useLoanRequestStatusLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      statusTypes.map((statusType: IStatusTypeLookup) => ({
        value: statusType.statusTypeId.toString(),
        label: statusType.statusTypeName,
      })),
    [statusTypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedStatusType = selectedOption
          ? statusTypes.find(
              (u: IStatusTypeLookup) =>
                u.statusTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedStatusType)
      }
    },
    [form, name, onChangeEvent, statusTypes, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading status types..."}
