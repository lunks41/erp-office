...
import { ICOACategory1Lookup } from "@/interfaces/lookup"
...
import { useCOACategory1Lookup } from "@/hooks/use-lookup"
...
export default function COACategory1Autocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICOACategory1Lookup | null) => void
}) {
  const { data: coacategory1s = [], isLoading } = useCOACategory1Lookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      coacategory1s.map((coacategory1: ICOACategory1Lookup) => ({
        value: coacategory1.coaCategoryId.toString(),
        label: coacategory1.coaCategoryName,
      })),
    [coacategory1s]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCOACategory1 = selectedOption
          ? coacategory1s.find(
              (u: ICOACategory1Lookup) =>
                u.coaCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCOACategory1)
      }
    },
    [form, name, onChangeEvent, coacategory1s, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    placeholder="Select COACategory1..."
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading coacategory1s..."}
