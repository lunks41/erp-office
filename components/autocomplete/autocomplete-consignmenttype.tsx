...
import { IConsignmentTypeLookup } from "@/interfaces/lookup"
...
import { useConsignmentTypeLookup } from "@/hooks/use-lookup"
...
export default function ConsignmentTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IConsignmentTypeLookup | null) => void
}) {
  const { data: consignmentTypes = [], isLoading } = useConsignmentTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      consignmentTypes.map((consignmentType: IConsignmentTypeLookup) => ({
        value: consignmentType.consignmentTypeId.toString(),
        label: consignmentType.consignmentTypeName,
      })),
    [consignmentTypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedConsignmentType = selectedOption
          ? consignmentTypes.find(
              (u: IConsignmentTypeLookup) =>
                u.consignmentTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedConsignmentType)
      }
    },
    [form, name, onChangeEvent, consignmentTypes, markOptionSelected]
    )
  )
