...
import { IEntityTypeLookup } from "@/interfaces/lookup"
...
import { useEntityTypeLookup } from "@/hooks/use-lookup"
...
export default function EntityTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IEntityTypeLookup | null) => void
}) {
  const { data: entityTypes = [], isLoading } = useEntityTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      entityTypes.map((entityType: IEntityTypeLookup) => ({
        value: entityType.entityTypeId.toString(),
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedEntityType = selectedOption
          ? entityTypes.find(
              (u: IEntityTypeLookup) =>
                u.entityTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedEntityType)
      }
    },
    [form, name, onChangeEvent, entityTypes, markOptionSelected]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading entityTypes..."}
