...
import { IServiceTypeCategoryLookup } from "@/interfaces/lookup"
...
export default function ServiceTypeCategoryAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IServiceTypeCategoryLookup | null) => void
}) {
  const { data: serviceTypeCategorys = [], isLoading } =
    useServiceTypeCategoryLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      serviceTypeCategorys.map(
        (serviceTypeCategory: IServiceTypeCategoryLookup) => ({
          value: serviceTypeCategory.serviceTypeCategoryId.toString(),
          label: serviceTypeCategory.serviceTypeCategoryName,
        })
      ),
    [serviceTypeCategorys]
  )
  ...
  const selectClassNames = React.useMemo(
    () => ({
      control: (state: { isFocused: boolean; isDisabled: boolean }) =>
        cn(
          "bservice-input data-[placeholder]:text-muted-foreground [&_svg:not([class*='text-'])]:text-muted-foreground focus-visible:bservice-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:bservice-destructive dark:bg-input/30 dark:hover:bg-input/50",
          "flex w-full items-center justify-between gap-2 rounded-md bservice bg-transparent pl-3 pr-0 py-2 text-sm whitespace-nowrap shadow-xs transition-[color,box-shadow] outline-none",
          "focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50",
          state.isFocused
            ? "bservice-ring ring-[3px] ring-ring/50"
            : "bservice-input",
      ...
      menu: () =>
        cn(
          ...
          "relative z-[9999] min-w-[8rem] overflow-hidden rounded-md bservice shadow-md animate-in fade-in-80",
          "mt-1"
        ),
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedServiceTypeCategory = selectedOption
          ? serviceTypeCategorys.find(
              (u: IServiceTypeCategoryLookup) =>
                u.serviceTypeCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedServiceTypeCategory)
      }
    },
    [form, name, onChangeEvent, serviceTypeCategorys]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    placeholder="Select ServiceTypeCategory..."
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading serviceTypeCategorys..."}
