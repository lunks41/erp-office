...
import { ILoanTypeLookup } from "@/interfaces/lookup"
...
import { useLoanTypeLookup } from "@/hooks/use-lookup"
...
export default function LoanTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ILoanTypeLookup | null) => void
}) {
  const { data: loanTypes = [], isLoading } = useLoanTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      loanTypes.map((loanType: ILoanTypeLookup) => ({
        value: loanType.loanTypeId.toString(),
        label: loanType.loanTypeName,
      })),
    [loanTypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedLoanType = selectedOption
          ? loanTypes.find(
              (u: ILoanTypeLookup) =>
                u.loanTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedLoanType)
      }
    },
    [form, name, onChangeEvent, loanTypes, markOptionSelected]
    )
  )
