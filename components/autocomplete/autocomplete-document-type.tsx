...
>({
  ...
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      documentTypes.map((documentType: IDocumentTypeLookup) => ({
        ...
        label: documentType.docTypeName,
      })),
    [documentTypes]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedDocumentType = selectedOption
          ? documentTypes.find(
              (u: IDocumentTypeLookup) =>
                u.docTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedDocumentType)
      }
    },
    [form, name, onChangeEvent, documentTypes, markOptionSelected]
  )
