...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IOrderTypeCategoryLookup | null) => void
}) {
  ...
  const options: FieldOption[] = React.useMemo(
    () =>
      orderTypeCategorys.map((orderTypeCategory: IOrderTypeCategoryLookup) => ({
        ...
        label: orderTypeCategory.orderTypeCategoryName,
      })),
    [orderTypeCategorys]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedOrderTypeCategory = selectedOption
          ? orderTypeCategorys.find(
              (u: IOrderTypeCategoryLookup) =>
                u.orderTypeCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedOrderTypeCategory)
      }
    },
    [form, name, onChangeEvent, orderTypeCategorys]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading orderTypeCategorys..."}
