...
import { IDesignationLookup } from "@/interfaces/lookup"
...
import { useDesignationLookup } from "@/hooks/use-lookup"
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IDesignationLookup | null) => void
}) {
  const { data: designations = [], isLoading } = useDesignationLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      designations.map((designation: IDesignationLookup) => ({
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedDesignation = selectedOption
          ? designations.find(
              (u: IDesignationLookup) =>
                u.designationId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedDesignation)
      }
    },
    [form, name, onChangeEvent, designations, markOptionSelected]
    )
  )
