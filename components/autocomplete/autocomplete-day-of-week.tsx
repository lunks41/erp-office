...
export default function DayOfWeekAutocomplete<
...
>({
  ...
  isRequired?: boolean
  onChangeEvent?: (
    selectedOption: { dayNumber: number; dayName: string } | null
  ) => void
}) {
  // Generate day options (1-7)
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedDay = selectedOption
          ? {
              dayNumber: Number(selectedOption.value),
              dayName: selectedOption.label,
            }
          : null
        onChangeEvent(selectedDay)
      }
    },
