...
import { IGenderLookup } from "@/interfaces/lookup"
...
import { useGenderLookup } from "@/hooks/use-lookup"
...
export default function GenderAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IGenderLookup | null) => void
}) {
  const { data: genders = [], isLoading } = useGenderLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      genders.map((gender: IGenderLookup) => ({
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedGender = selectedOption
          ? genders.find(
              (u: IGenderLookup) =>
                u.genderId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedGender)
      }
    },
    [form, name, onChangeEvent, genders, markOptionSelected]
    )
  )
