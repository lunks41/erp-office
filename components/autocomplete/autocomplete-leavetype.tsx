...
export default function LeaveTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ILeaveTypeLookup | null) => void
}) {
  const { data: leaveTypes = [], isLoading } = useLeaveTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      leaveTypes.map((leaveType: ILeaveTypeLookup) => ({
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedLeaveType = selectedOption
          ? leaveTypes.find(
              (u: ILeaveTypeLookup) =>
                u.leaveTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedLeaveType)
      }
    },
    [form, name, onChangeEvent, leaveTypes, markOptionSelected]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading leaveTypes..."}
