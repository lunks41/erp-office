...
import { IModuleLookup } from "@/interfaces/lookup"
...
export default function ModuleAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IModuleLookup | null) => void
  IsVisible?: boolean
  IsMandatory?: boolean
}) {
  const { data: modules = [], isLoading } = useModuleLookup(
    IsVisible,
    IsMandatory
  )
  ...
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      modules.map((module: IModuleLookup) => ({
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedModule = selectedOption
          ? modules.find(
              (u: IModuleLookup) =>
                u.moduleId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedModule)
      }
    },
    [form, name, onChangeEvent, modules, markOptionSelected]
    )
  )
