...
import { ITaxCategoryLookup } from "@/interfaces/lookup"
...
export default function TaxCategoryAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ITaxCategoryLookup | null) => void
}) {
  const { data: taxCategorys = [], isLoading } = useTaxCategoryLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      taxCategorys.map((taxCategory: ITaxCategoryLookup) => ({
        value: taxCategory.taxCategoryId.toString(),
        label: taxCategory.taxCategoryName,
      })),
    [taxCategorys]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedTaxCategory = selectedOption
          ? taxCategorys.find(
              (u: ITaxCategoryLookup) =>
                u.taxCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedTaxCategory)
      }
    },
    [form, name, onChangeEvent, taxCategorys, markOptionSelected]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading taxCategorys..."}
