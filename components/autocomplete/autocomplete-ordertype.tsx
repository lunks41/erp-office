...
import { IOrderTypeLookup } from "@/interfaces/lookup"
...
import { useOrderTypeLookup } from "@/hooks/use-lookup"
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IOrderTypeLookup | null) => void
}) {
  const { data: orderTypes = [], isLoading } = useOrderTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      orderTypes.map((orderType: IOrderTypeLookup) => ({
        value: orderType.orderTypeId.toString(),
        label: orderType.orderTypeName,
      })),
    [orderTypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedOrderType = selectedOption
          ? orderTypes.find(
              (u: IOrderTypeLookup) =>
                u.orderTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedOrderType)
      }
    },
    [form, name, onChangeEvent, orderTypes, markOptionSelected]
    )
  )
