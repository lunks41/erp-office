...
import { IEmployerLookup } from "@/interfaces/lookup"
...
import { useEmployerLookup } from "@/hooks/use-lookup"
...
>({
  ...
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      employers.map((employer: IEmployerLookup) => ({
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedEmployer = selectedOption
          ? employers.find(
              (employer: IEmployerLookup) =>
                employer.employerId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedEmployer)
      }
    },
    [form, name, onChangeEvent, employers, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading employers..."}
