...
import { IWorkLocationLookup } from "@/interfaces/lookup"
...
export default function WorkLocationAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IWorkLocationLookup | null) => void
}) {
  const { data: workLocations = [], isLoading } = useWorkLocationLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      workLocations
        .filter(
          (workLocation: IWorkLocationLookup) =>
            workLocation &&
            workLocation.workLocationId !== undefined &&
            workLocation.workLocationName
        )
        .map((workLocation: IWorkLocationLookup) => ({
          value: workLocation.workLocationId.toString(),
          label: workLocation.workLocationName,
        })),
    [workLocations]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedWorkLocation = selectedOption
          ? workLocations.find(
              (u: IWorkLocationLookup) =>
                u.workLocationId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedWorkLocation)
      }
    },
    [form, name, onChangeEvent, workLocations, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading work locations..."}
