...
import { ICreditTermLookup } from "@/interfaces/lookup"
...
import { useCreditTermLookup } from "@/hooks/use-lookup"
...
export default function CreditTermAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICreditTermLookup | null) => void
}) {
  const { data: creditterms = [], isLoading } = useCreditTermLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      creditterms.map((creditterm: ICreditTermLookup) => ({
        value: creditterm.creditTermId.toString(),
        label: creditterm.creditTermName,
      })),
    [creditterms]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCreditTerm = selectedOption
          ? creditterms.find(
              (u: ICreditTermLookup) =>
                u.creditTermId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCreditTerm)
      }
    },
    [form, name, onChangeEvent, creditterms, markOptionSelected]
  )
