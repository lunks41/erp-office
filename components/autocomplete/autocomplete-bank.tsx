...
import { IBankLookup } from "@/interfaces/lookup"
import {
  IconCheck,
  IconChevronDown,
  IconRefresh,
...
import { useBankLookup } from "@/hooks/use-lookup"
...
export default function BankAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IBankLookup | null) => void
}) {
  const { data: banks = [], isLoading, refetch } = useBankLookup()
  ...
  // Create options with selected bank at top
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedBank = selectedOption
          ? banks.find(
              (u: IBankLookup) => u.bankId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedBank)
      }
    },
    [form, name, onChangeEvent, banks, markOptionSelected]
  )
