...
import { useUserLookup } from "@/hooks/use-lookup"
...
export default function UserAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedUser = selectedOption
          ? users.find(
              (u: IUserLookup) => u.userId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedUser)
      }
    },
    [form, name, onChangeEvent, users, markOptionSelected]
    )
  )
