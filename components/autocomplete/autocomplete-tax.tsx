...
import { ITaxLookup } from "@/interfaces/lookup"
...
import { useTaxLookup } from "@/hooks/use-lookup"
...
export default function TaxAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ITaxLookup | null) => void
}) {
  const { data: taxs = [], isLoading } = useTaxLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      taxs.map((tax: ITaxLookup) => ({
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedTax = selectedOption
          ? taxs.find(
              (u: ITaxLookup) => u.taxId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedTax)
      }
    },
    [form, name, onChangeEvent, taxs, markOptionSelected]
    )
  )
