...
import { IEmployeeLookup } from "@/interfaces/lookup"
...
import { useEmployeeLookup } from "@/hooks/use-lookup"
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IEmployeeLookup | null) => void
}) {
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedEmployee = selectedOption
          ? employees.find(
              (u: IEmployeeLookup) =>
                u.employeeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedEmployee)
      }
    },
    [form, name, onChangeEvent, employees, markOptionSelected]
    )
  )
