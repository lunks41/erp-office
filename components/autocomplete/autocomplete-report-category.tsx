...
import { IReportCategoryLookup } from "@/interfaces/lookup"
...
interface ReportCategoryAutocompleteProps<T extends Record<string, unknown>> {
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IReportCategoryLookup | null) => void
}

export default function ReportCategoryAutocomplete<
...
>({
  ...
  isRequired = false,
  onChangeEvent,
}: ReportCategoryAutocompleteProps<T>) {
  const { data: categories = [], isLoading } = useReportCategoryLookup()

  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      categories.map((c: IReportCategoryLookup) => ({
        value: c.repCategoryId.toString(),
        label:
          c.repCategoryCode && c.repCategoryName
            ? `${c.repCategoryCode} - ${c.repCategoryName}`
            : c.repCategoryName || String(c.repCategoryId),
      })),
    [categories]
  )
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const row = selectedOption
          ? categories.find(
              (u: IReportCategoryLookup) =>
                u.repCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(row)
      }
