...
import { IJobStatusLookup } from "@/interfaces/lookup"
...
export default function StatusAutocomplete<T extends Record<string, unknown>>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IJobStatusLookup | null) => void
}) {
  const { data: jobstatuss = [], isLoading } = useJobStatusLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      jobstatuss.map((status: IJobStatusLookup) => ({
        value: status.jobStatusId.toString(),
        label: status.jobStatusName,
      })),
    [jobstatuss]
  )
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedStatus = selectedOption
          ? jobstatuss.find(
              (u: IJobStatusLookup) =>
                u.jobStatusId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedStatus)
      }
    },
    [form, name, onChangeEvent, jobstatuss, markOptionSelected]
    )
  )
