...
import { IPassTypeLookup } from "@/interfaces/lookup"
...
import { usePassTypeLookup } from "@/hooks/use-lookup"
...
export default function PassTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IPassTypeLookup | null) => void
}) {
  const { data: passTypes = [], isLoading } = usePassTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      passTypes.map((passType: IPassTypeLookup) => ({
        value: passType.passTypeId.toString(),
        label: passType.passTypeName,
      })),
    [passTypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedPassType = selectedOption
          ? passTypes.find(
              (u: IPassTypeLookup) =>
                u.passTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedPassType)
      }
    },
    [form, name, onChangeEvent, passTypes, markOptionSelected]
    )
  )
