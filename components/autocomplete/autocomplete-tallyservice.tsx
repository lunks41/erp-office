...
import { useTallyServiceLookup } from "@/hooks/use-lookup"
...
export default function TallyServiceAutocomplete<
...
>({
  ...
  const { data: tallyServices = [], isLoading, refetch } = useTallyServiceLookup()
  ...
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      tallyServices.map((jobOrder: ITallyServiceLookup) => ({
