...
import { IPaymentTypeLookup } from "@/interfaces/lookup"
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IPaymentTypeLookup | null) => void
}) {
  const { data: paymentTypes = [], isLoading } = usePaymentTypeLookup()
  // Memoize options to prevent unnecessary recalculations
  const options: FieldOption[] = React.useMemo(
    () =>
      paymentTypes.map((paymentType: IPaymentTypeLookup) => ({
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedPaymentType = selectedOption
          ? paymentTypes.find(
              (u: IPaymentTypeLookup) =>
                u.paymentTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedPaymentType)
      }
    },
    [form, name, onChangeEvent, paymentTypes, markOptionSelected]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading paymentTypes..."}
