...
export default function VesselTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IVesselTypeLookup | null) => void
}) {
  const { data: vesselTypes = [], isLoading, refetch } = useVesselTypeLookup()
  ...
  const handleRefresh = React.useCallback(async () => {
    try {
      ...
      console.error("Error refreshing vessel types:", error)
    }
  }, [refetch])
  ...
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      vesselTypes.map((vesselType: IVesselTypeLookup) => ({
        value: vesselType.vesselTypeId.toString(),
        label: vesselType.vesselTypeCode + " - " + vesselType.vesselTypeName,
      })),
    [vesselTypes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedVesselType = selectedOption
          ? vesselTypes.find(
              (u: IVesselTypeLookup) =>
                u.vesselTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedVesselType)
      }
    },
    [form, name, onChangeEvent, vesselTypes, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading vessel types..."}
