...
import { ISubCategoryLookup } from "@/interfaces/lookup"
...
import { useSubCategoryLookup } from "@/hooks/use-lookup"
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ISubCategoryLookup | null) => void
}) {
  const { data: subCategorys = [], isLoading } = useSubCategoryLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      subCategorys.map((subCategory: ISubCategoryLookup) => ({
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedSubCategory = selectedOption
          ? subCategorys.find(
              (u: ISubCategoryLookup) =>
                u.subCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedSubCategory)
      }
    },
    [form, name, onChangeEvent, subCategorys, markOptionSelected]
    )
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleSearchableKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading subCategorys..."}
