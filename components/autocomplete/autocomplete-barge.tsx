...
import { useBargeLookup } from "@/hooks/use-lookup"
...
export default function BargeAutocomplete<T extends Record<string, unknown>>({
  ...
  // Create options with selected barge at top
  ...
  const handleChange = React.useCallback(
    ...
    [form, name, onChangeEvent, barges, markOptionSelected]
  )
  ...
  const handleMenuOpen = React.useCallback(() => {
    ...
    setTimeout(() => {
      ...
      if (selectedValue) {
        ...
        const selectors = [
          `[id*="${name || "barge-select"}"] .react-select__menu-list`,
