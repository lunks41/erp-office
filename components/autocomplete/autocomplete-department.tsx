...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: IDepartmentLookup | null) => void
}) {
  const { data: departments = [], isLoading } = useDepartmentLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      departments.map((department: IDepartmentLookup) => ({
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedDepartment = selectedOption
          ? departments.find(
              (u: IDepartmentLookup) =>
                u.departmentId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedDepartment)
      }
    },
    [form, name, onChangeEvent, departments, markOptionSelected]
    )
  )
