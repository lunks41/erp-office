...
import { ICOACategory2Lookup } from "@/interfaces/lookup"
...
import { useCOACategory2Lookup } from "@/hooks/use-lookup"
...
export default function COACategory2Autocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICOACategory2Lookup | null) => void
}) {
  const { data: coacategory2s = [], isLoading } = useCOACategory2Lookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      coacategory2s.map((coacategory2: ICOACategory2Lookup) => ({
        value: coacategory2.coaCategoryId.toString(),
        label: coacategory2.coaCategoryName,
      })),
    [coacategory2s]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCOACategory2 = selectedOption
          ? coacategory2s.find(
              (u: ICOACategory2Lookup) =>
                u.coaCategoryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCOACategory2)
      }
    },
    [form, name, onChangeEvent, coacategory2s, markOptionSelected]
    )
  )
