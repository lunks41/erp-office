...
import { ICountryLookup } from "@/interfaces/lookup"
...
export default function CountryAutocomplete<T extends Record<string, unknown>>({
  form,
  label,
  name,
  isDisabled = false,
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICountryLookup | null) => void
}) {
  const { data: countries = [], isLoading, refetch } = useCountryLookup()
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCountry = selectedOption
          ? countries.find(
              (u: ICountryLookup) =>
                u.countryId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCountry)
      }
    },
    [form, name, onChangeEvent, countries, markOptionSelected]
    )
  )
