...
import { ITransportModeLookup } from "@/interfaces/lookup"
...
import { useTransportModeLookup } from "@/hooks/use-lookup"
...
export default function TransportModeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ITransportModeLookup | null) => void
}) {
  const { data: transportModes = [], isLoading } = useTransportModeLookup()
  // Memoize options to prevent unnecessary recalculations
  const baseOptions: FieldOption[] = React.useMemo(
    () =>
      transportModes.map((modeType: ITransportModeLookup) => ({
        value: modeType.transportModeId.toString(),
        label: modeType.transportModeName,
      })),
    [transportModes]
  )
...
const handleChange = wrapOnChange(
    ...
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedTransportMode = selectedOption
          ? transportModes.find(
              (u: ITransportModeLookup) =>
                u.transportModeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedTransportMode)
      }
    },
    [form, name, onChangeEvent, transportModes, markOptionSelected]
    )
  )
