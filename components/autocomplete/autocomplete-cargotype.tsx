...
import { ICargoTypeLookup } from "@/interfaces/lookup"
import {
  IconCheck,
  IconChevronDown,
  IconRefresh,
...
import { useCargoTypeLookup } from "@/hooks/use-lookup"
...
export default function CargoTypeAutocomplete<
...
>({
  ...
  isDisabled?: boolean
  isRequired?: boolean
  onChangeEvent?: (selectedOption: ICargoTypeLookup | null) => void
}) {
  const { data: cargoTypes = [], isLoading, refetch } = useCargoTypeLookup()
  ...
  const handleRefresh = React.useCallback(async () => {
    try {
      ...
      console.error("Error refreshing cargo types:", error)
    }
  }, [refetch])
  ...
  const options: FieldOption[] = React.useMemo(
    () =>
      cargoTypes.map((cargoType: ICargoTypeLookup) => ({
        value: cargoType.cargoTypeId.toString(),
        label: cargoType.cargoTypeName,
      })),
    [cargoTypes]
  )
  ...
  const handleChange = React.useCallback(
    (option: SingleValue<FieldOption> | MultiValue<FieldOption>) => {
      ...
      if (onChangeEvent) {
        const selectedCargoType = selectedOption
          ? cargoTypes.find(
              (u: ICargoTypeLookup) =>
                u.cargoTypeId.toString() === selectedOption.value
            ) || null
          : null
        onChangeEvent(selectedCargoType)
      }
    },
    [form, name, onChangeEvent, cargoTypes, markOptionSelected]
  )
              ...
              <FormItem className={cn("flex flex-col", className)}>
                <div ref={selectControlRef} onKeyDown={handleKeyDown}>
                  <Select
                    ...
                    menuPosition="fixed"
                    isLoading={isLoading}
                    loadingMessage={() => "Loading cargo types..."}
