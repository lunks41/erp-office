...
import { DocumentOperationsTableActions } from "./table-document-action-operations"
import { DocumentOperationsTableHeader } from "./table-document-header-operations"

interface DocumentOperationsTableProps<T> {
  data: T[]
  columns: ColumnDef<T>[]
  isLoading?: boolean
  moduleId?: number
...
export function DocumentOperationsTable<T>({
  data,
  columns,
  isLoading,
  moduleId,
  ...
  initialSelectedIds = [],
}: DocumentOperationsTableProps<T>) {
  ...
  // State to track if order has changed
  const [hasOrderChanged, setHasOrderChanged] = useState(false)
  const [_originalData, setOriginalData] = useState(data)

  // Update original data when data changes
  useEffect(() => {
            ...
            cell: ({ row }: { row: Row<T> }) => {
              ...
              return (
                <div className="flex min-w-0 items-center gap-0.5 overflow-hidden">
                  ...
                  {/* Action Buttons */}
                  <DocumentOperationsTableActions
