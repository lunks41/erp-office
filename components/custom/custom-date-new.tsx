...
export const CustomDateNew = <T extends FieldValues = FieldValues>({
  ...
  const parseUserInput = React.useCallback(
    (input: string | Date | null | undefined): Date | undefined => {
      ...
      const formats = [
        ...
        `${decimalDateFormat} HH:mm:ss`,
        `${decimalDateFormat} HH:mm`,
  ...
  const calendarClassName = React.useMemo(() => {
    if (size === "lg") return "[--cell-size:2.25rem]"
    if (size === "default") return "[--cell-size:2rem]"
    return "[--cell-size:2rem]"
  }, [size])
              ...
              <FormControl>
                <div className="relative flex gap-2">
                  ...
                  <Popover open={open} onOpenChange={setOpen}>
                    ...
                    <PopoverContent
                      className="z-100 w-auto overflow-hidden p-0 shadow-md"
                      align="start"
                      side="bottom"
                      sideOffset={6}
                      collisionPadding={12}
