...
export function DefaultForm() {
  ...
  useEffect(() => {
    if (
      ...
      const { result, message, data } =
        defaultSettingResponse as DefaultSettingResponse
      ...
      if (result === -1) {
        toast.error(message || "Failed to load default settings")
        return
      }
  ...
  }, [defaultSettingResponse, form, isLoadingChartOfAccounts, chartOfAccounts])
  ...
  function handleConfirmSave() {
    ...
    saveDefaultSettings(data, {
      onSuccess: (response) => {
        const { result, message } = response as DefaultSettingResponse
        ...
        if (result === -1) {
          toast.error(message || "Failed to save default settings")
          return
        }

        if (result === 1) {
          toast.success(message || "Default settings saved successfully")
  ...
  if (isError) {
    return (
      <div className="flex flex-col items-center justify-center space-y-4 p-8">
        <p className="text-destructive">Failed to load default settings</p>
  ...
  return (
    <div className="rounded-lg border p-4">
      {defaultSettingResponse?.result === -2 ? (
