...
export default function DebitNoteForm({
  ...
  const resolveLineTaskId = useCallback(
    (detailTaskId?: number | null) => {
      if (detailTaskId && detailTaskId > 0) return detailTaskId
  ...
  const { data: chartOfAccounts = [] } = useChartOfAccountLookup(
    Number(companyId)
  )
  ...
  const applyGlLookup = useCallback(
    (glId?: number | null) => {
      if (!glId || glId <= 0) {
        form.setValue("glId", 0, { shouldDirty: false })
        form.setValue("glCode", "", { shouldDirty: false })
        form.setValue("glName", "", { shouldDirty: false })
        return
      }
      const glData = chartOfAccounts.find(
        (gl: IChartOfAccountLookup) => gl.glId === glId
      )
      form.setValue("glId", glId, { shouldDirty: false })
      form.setValue("glCode", glData?.glCode ?? "", { shouldDirty: false })
      form.setValue("glName", glData?.glName ?? "", { shouldDirty: false })
    },
  ...
  useEffect(() => {
    ...
    if (editingDetail) {
      const gstId = Number(editingDetail.gstId) || 1
      form.reset({
        ...
        taskId: resolveLineTaskId(editingDetail.taskId),
  ...
  useEffect(() => {
    const glId = Number(form.getValues("glId")) || 0
    if (glId <= 0 || !chartOfAccounts.length) return
    if (form.getValues("glCode")) return
    applyGlLookup(glId)
  }, [chartOfAccounts.length, applyGlLookup, form])
  ...
  const handleChargeChange = useCallback(
    (selectedCharge: IChargeLookup | null) => {
      if (selectedCharge && selectedCharge.chargeName) {
        ...
        if (selectedCharge.glId && selectedCharge.glId > 0) {
          applyGlLookup(selectedCharge.glId)
        }
    ...
    [applyGlLookup, form, onChargeChange, taskId]
  )

  const handleChartOfAccountChange = useCallback(
    (selectedGl: IChartOfAccountLookup | null) => {
      if (selectedGl?.glId) {
        form.setValue("glId", selectedGl.glId, { shouldDirty: true })
        form.setValue("glCode", selectedGl.glCode || "", { shouldDirty: true })
        form.setValue("glName", selectedGl.glName || "", { shouldDirty: true })
        return
      }
      applyGlLookup(0)
    },
  ...
  const onSubmit = (data: DebitNoteDtSchemaType) => {
    ...
    submitAction(
      normalizeDebitNoteLineTotals(
        {
          ...data,
          taskId: resolveLineTaskId(data.taskId),
          gstId: Number(data.gstId) > 0 ? Number(data.gstId) : 1,
          glId,
        },
        { amtDec }
      )
  ...
  const onInvalid = (errors: FieldErrors<DebitNoteDtSchemaType>) => {
        ...
        <div className="w-[88%] min-w-0">
          <div className="space-y-2">
            ...
            <div className="grid grid-cols-10 gap-2">
              ...
              <div className="col-span-1 border-l border-border pl-2">
                <GSTAutocomplete
                  form={form}
                  name="gstId"
            ...
            {/* Row 2: GL account, service charge, remarks */}
