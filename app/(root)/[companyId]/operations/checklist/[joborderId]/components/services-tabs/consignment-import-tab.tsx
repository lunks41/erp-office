...
export function ConsignmentImportTab({
  ...
  const transactionId = OperationsTransactionId.consignmentImport
  ...
  const [bulkDeleteConfirmation, setBulkDeleteConfirmation] = useState<{
    isOpen: boolean
    consignmentImportIds: string[]
    jobOrderId: number | null
    count: number
  }>({
    isOpen: false,
    consignmentImportIds: [],
    jobOrderId: null,
    count: 0,
  })
  ...
  // Clone single Consignment Import confirmation state
  const [cloneSaveConfirmation, setCloneSaveConfirmation] = useState<{
    isOpen: boolean
    sourceItem: IConsignmentImport | null
  }>({
    isOpen: false,
    sourceItem: null,
  })
  ...
  const handleConfirmBulkDelete = async () => {
    ...
    try {
      ...
      const response = await postData(
        `${JobOrder_ConsignmentImport.bulkDelete}/${bulkDeleteConfirmation.jobOrderId}`,
        {
          consignmentImportIds: bulkDeleteConfirmation.consignmentImportIds,
        }
      )
      ...
      if (response.result === 1) {
        ...
        onTaskAdded?.()
        toast.success(
          `Successfully deleted ${bulkDeleteConfirmation.consignmentImportIds.length} item(s)`
        )
  ...
  const handleCloneRow = useCallback((item: IConsignmentImport) => {
  ...
  const handleSubmit = useCallback(
    async (formData: Partial<IConsignmentImport>) => {
      try {
        ...
        const processedData = {
          ...formData,
          receiveDate: formatDateForApi(formData.receiveDate) || undefined,
          deliverDate: formatDateForApi(formData.deliverDate) || undefined,
          arrivalDate: formatDateForApi(formData.arrivalDate) || undefined,
        }
  ...
  const handleCloneTaskClick = useCallback((selectedIds: string[]) => {
    if (selectedIds.length === 0) {
      toast.error("Please select at least one consignment import to clone")
      return
    }
  ...
  const handleCloneTask = async () => {
    ...
    try {
      ...
      const cloneData = {
        ...
        fromTaskId: Task.ConsignmentImport,
      ...
      if (response.data.result === 1) {
        toast.success(
          response.data.message ||
            "Consignment import cloned to different company successfully!"
        )
        ...
        toast.error(
          response.data.message || "Failed to clone consignment import"
        )
      }
    } catch (error) {
      console.error(
        "Error cloning consignment import to different company:",
        error
      )
      toast.error("Failed to clone consignment import. Please try again.")
    } finally {
      setIsCloning(false)
    }
  }
  ...
  return (
    <>
      ...
      <DeleteConfirmation
        ...
        title="Delete Multiple Consignment Imports"
        description="This action cannot be undone. This will permanently delete the selected consignment imports from our servers."
        itemName={`${bulkDeleteConfirmation.count} consignment import${bulkDeleteConfirmation.count !== 1 ? "s" : ""}`}
      ...
      <SaveConfirmation
        ...
        title="Clone Consignment Import"
        itemName={`Consignment Import ${
        ...
        onConfirm={async () => {
          ...
          const submitData: ConsignmentImportSchemaType = {
            ...(src as unknown as ConsignmentImportSchemaType),
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {selectedItems.length} selected consignment import item(s).
            </DialogDescription>
          </DialogHeader>
          <Form {...cloneTaskForm}>
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              import item(s) to the selected company and job order? This action
