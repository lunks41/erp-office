...
export function FreshWaterTab({
  ...
  const transactionId = OperationsTransactionId.freshWater
  ...
  const handleConfirmSave = useCallback(async () => {
    ...
    try {
      const processedData = {
        ...
        chargeName: saveConfirmation.formData.chargeName || "",
        bargeName: saveConfirmation.formData.bargeName || "",
        uomName: saveConfirmation.formData.uomName || "",
        taskStatusName: saveConfirmation.formData.taskStatusName || "",
        remarks: saveConfirmation.formData.remarks || "",
  ...
  const handleCloneTaskClick = useCallback((selectedIds: string[]) => {
    if (selectedIds.length === 0) {
      toast.error("Please select at least one fresh water to clone")
      return
    }
  ...
  const handlePurchase = useCallback(
    (freshWaterId: string) => {
      const item = data?.find(
        (service) => service.freshWaterId.toString() === freshWaterId
      )
      ...
      if (response.data.result === 1) {
        toast.success(
          response.data.message ||
            "Fresh water cloned to different company successfully!"
        )
        ...
        toast.error(response.data.message || "Failed to clone fresh water")
      }
    } catch (error) {
      console.error("Error cloning fresh water to different company:", error)
      toast.error("Failed to clone fresh water. Please try again.")
    } finally {
      setIsCloning(false)
    }
  }
    ...
    <>
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        >
          <DialogHeader>
            ...
            <DialogDescription>
              {modalMode === "create"
                ? "Add a new fresh water record to this job order."
                : modalMode === "edit"
                  ? "Update the fresh water details."
                  : "View fresh water details (read-only)."}
            </DialogDescription>
          </DialogHeader>
          <Separator />
          <FreshWaterForm
      ...
      {showPurchaseModal && (
        <PurchaseDialog
          ...
          title="Purchase"
          description="Manage purchase details for this fresh water."
          jobOrderId={jobData.jobOrderId}
          taskId={Task.FreshWater}
        ...
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              ...
              {selectedItems.length} selected fresh water item(s).
            </DialogDescription>
          </DialogHeader>
          <Form {...cloneTaskForm}>
      ...
      >
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              Are you sure you want to clone {selectedItems.length} fresh water
