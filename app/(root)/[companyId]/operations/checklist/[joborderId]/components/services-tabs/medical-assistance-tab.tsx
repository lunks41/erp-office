...
export function MedicalAssistanceTab({
  ...
  const [bulkDeleteConfirmation, setBulkDeleteConfirmation] = useState<{
    isOpen: boolean
    medicalAssistanceIds: string[]
    jobOrderId: number | null
    count: number
  }>({
    isOpen: false,
    medicalAssistanceIds: [],
    jobOrderId: null,
    count: 0,
  })
  ...
  const handleBulkDelete = useCallback(
    (selectedIds: string[]) => {
      if (selectedIds.length === 0) {
        toast.error("Please select at least one medical assistance to delete")
        return
      }
      ...
      setBulkDeleteConfirmation({
        isOpen: true,
        medicalAssistanceIds: selectedIds,
        jobOrderId: jobData.jobOrderId,
  ...
  const handleConfirmBulkDelete = async () => {
    if (
      bulkDeleteConfirmation.medicalAssistanceIds.length === 0 ||
    ...
    try {
      ...
      const response = await postData(
        `${JobOrder_MedicalAssistance.bulkDelete}/${bulkDeleteConfirmation.jobOrderId}`,
        {
          medicalAssistanceIds: bulkDeleteConfirmation.medicalAssistanceIds,
        }
      )
      ...
      if (response.result === 1) {
        ...
        onTaskAdded?.()
        toast.success(
          `Successfully deleted ${bulkDeleteConfirmation.medicalAssistanceIds.length} item(s)`
        )
  ...
  const handleSubmit = useCallback(
    async (formData: Partial<IMedicalAssistance>) => {
      try {
        ...
        const processedData = {
          ...formData,
          admittedDate: formatDateForApi(formData.admittedDate) || undefined,
          dischargedDate:
            formatDateForApi(formData.dischargedDate) || undefined,
        }
  ...
  return (
    <>
      <div className="space-y-4">
        <div className="overflow-x-auto">
          <MedicalAssistanceTable
            ...
            onBulkDeleteMedicalAssistance={handleBulkDelete}
      ...
      <Dialog open={isModalOpen} onOpenChange={setIsModalOpen}>
        ...
        >
          ...
          {modalMode === "edit" || modalMode === "view" ? (
            <Tabs defaultValue="medicalAssistance">
              <TabsList className="mb-3">
                <TabsTrigger value="medicalAssistance">
                  Medical Assistance
                </TabsTrigger>
                <TabsTrigger value="transportation">Transportation</TabsTrigger>
              </TabsList>

              <TabsContent value="medicalAssistance">
      ...
      <DeleteConfirmation
        ...
        title="Delete Multiple Medical Assistance"
        description="This action cannot be undone. This will permanently delete the selected medical assistance items from our servers."
        itemName={`${bulkDeleteConfirmation.count} medical assistance item${bulkDeleteConfirmation.count !== 1 ? "s" : ""}`}
