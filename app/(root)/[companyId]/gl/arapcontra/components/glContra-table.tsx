...
export interface ContraTableProps {
  onContraSelect: (selectedContra: IGLContraHd | undefined) => void
...
export default function ContraTable({
  ...
  const {
    ...
    refetch: _refetchContra,
  } = useGetWithDatesAndPagination<IGLContraHd>(
    `${GLContra.get}`,
    TableName.glContra,
    searchQuery,
    searchStartDate ?? "",
  ...
  const data = contraResponse?.data || []
  const totalRecords = contraResponse?.totalRecords || data.length
  const isLoading = isLoadingContra || isRefetchingContra

  const getContraStatus = (isCancel: boolean) => {
    if (isCancel) {
      return "Cancelled"
    }
    return ""
