...
import { getDefaultValues } from "./glJournal-defaultvalues"

export interface GLJournalDetailsFormRef {
  recalculateAmounts: (
    exchangeRate?: number,
...
const GLJournalDetailsForm = React.forwardRef<
  GLJournalDetailsFormRef,
  GLJournalDetailsFormProps
>(
  (
    {
      Hdform,
    ...
    const defaultGLJournalDetails = useMemo(
    ...
    const createDefaultValues = (itemNo: number): GLJournalDtSchemaType => {
      // Use defaults if available, otherwise use defaultGLJournalDetails values
      const glId =
        defaultGlId && defaultGlId > 0
          ? defaultGlId
          : defaultGLJournalDetails.glId
      const gstId =
        defaultGstId && defaultGstId > 0
          ? defaultGstId
          : defaultGLJournalDetails.gstId

      return {
        ...defaultGLJournalDetails,
        itemNo,
        seqNo: itemNo,
        glId,
        gstId,
    ...
    const form = useForm<GLJournalDtSchemaType>({
      ...
      defaultValues: editingDetail
        ? {
            journalId: editingDetail.journalId ?? "0",
            journalNo: editingDetail.journalNo ?? "",
    ...
    const populateCodeNameFields = (
      formData: GLJournalDtSchemaType
    ): GLJournalDtSchemaType => {
    ...
    const onSubmit = async (_data: GLJournalDtSchemaType) => {
      try {
        ...
        if (!validationResult.success) {
          ...
          validationResult.error.issues.forEach((issue) => {
            const fieldPath = issue.path.join(
              "."
            ) as keyof GLJournalDtSchemaType
        ...
        const rowData: IGLJournalDt = {
          journalId: updatedData.journalId ?? "0",
          journalNo: updatedData.journalNo ?? "",
          ...
          >
            ...
            <div className="col-span-8 mb-1">
              <div className="flex items-center gap-3">
                ...
                >
                  {isCancelled
                    ? "Details (Disabled - GLJournal Cancelled)"
                    : editingDetail
...
GLJournalDetailsForm.displayName = "GLJournalDetailsForm"

export default GLJournalDetailsForm
