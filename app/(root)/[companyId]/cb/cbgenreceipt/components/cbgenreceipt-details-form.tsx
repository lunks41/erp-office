...
import { getDefaultValues } from "./cbgenreceipt-defaultvalues"

export interface CbGenReceiptDetailsFormRef {
  recalculateAmounts: (
    exchangeRate?: number,
...
interface CbGenReceiptDetailsFormProps {
...
const CbGenReceiptDetailsForm = React.forwardRef<
  CbGenReceiptDetailsFormRef,
  CbGenReceiptDetailsFormProps
>(
  (
    {
      Hdform,
    ...
    const defaultCbGenReceiptDetails = useMemo(
    ...
    const createDefaultValues = (itemNo: number): CbGenReceiptDtSchemaType => {
      // Use defaults if available, otherwise use defaultCbGenReceiptDetails values
      const glId =
        defaultGlId && defaultGlId > 0
          ? defaultGlId
          : defaultCbGenReceiptDetails.glId
      const gstId =
        defaultGstId && defaultGstId > 0
          ? defaultGstId
          : defaultCbGenReceiptDetails.gstId

      return {
        ...defaultCbGenReceiptDetails,
        itemNo,
        seqNo: itemNo,
        glId,
        gstId,
    ...
    const populateCodeNameFields = (
      formData: CbGenReceiptDtSchemaType
    ): CbGenReceiptDtSchemaType => {
    ...
    // Uses reusable helper function from gl-journal-calculations
    ...
    const onSubmit = async (_data: CbGenReceiptDtSchemaType) => {
      try {
        ...
        if (!validationResult.success) {
          ...
          validationResult.error.issues.forEach((issue) => {
            const fieldPath = issue.path.join(
              "."
            ) as keyof CbGenReceiptDtSchemaType
        ...
        const rowData: ICbGenReceiptDt = {
          receiptId: updatedData.receiptId ?? "0",
          receiptNo: updatedData.receiptNo ?? "",
          ...
          >
            ...
            <div className="col-span-8 mb-1">
              <div className="flex items-center gap-3">
                ...
                >
                  {isCancelled
                    ? "Details (Disabled - CbGenReceipt Cancelled)"
                    : editingDetail
...
CbGenReceiptDetailsForm.displayName = "CbGenReceiptDetailsForm"

export default CbGenReceiptDetailsForm
