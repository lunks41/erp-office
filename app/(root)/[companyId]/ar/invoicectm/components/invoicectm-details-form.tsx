...
import { getDefaultValues } from "./invoicectm-defaultvalues"

export interface InvoiceCtmDetailsFormRef {
  recalculateAmounts: (
    exchangeRate?: number,
...
interface InvoiceCtmDetailsFormProps {
  Hdform: UseFormReturn<ArInvoiceCtmHdSchemaType>
  onAddRowAction?: (rowData: IArInvoiceCtmDt) => void
  onCancelEdit?: () => void
  editingDetail?: ArInvoiceCtmDtSchemaType | null
  visible: IVisibleFields
  required: IMandatoryFields
  companyId: number
  existingDetails?: ArInvoiceCtmDtSchemaType[]
  defaultGlId?: number
  defaultUomId?: number
  defaultGstId?: number
  isCancelled?: boolean
...
const InvoiceCtmDetailsForm = React.forwardRef<
  InvoiceCtmDetailsFormRef,
  InvoiceCtmDetailsFormProps
>(
  (
    {
      Hdform,
    ...
    const createDefaultValues = (itemNo: number): ArInvoiceCtmDtSchemaType => {
    ...
    const form = useForm<ArInvoiceCtmDtSchemaType>({
      resolver: zodResolver(ArInvoiceCtmDtSchema(required, visible)),
      mode: "onSubmit",
    ...
    const populateCodeNameFields = (
      formData: ArInvoiceCtmDtSchemaType
    ): ArInvoiceCtmDtSchemaType => {
    ...
    const onSubmit = async (_data: ArInvoiceCtmDtSchemaType) => {
      try {
        ...
        const validationResult = ArInvoiceCtmDtSchema(
          required,
          visible
        ...
        if (!validationResult.success) {
          ...
          validationResult.error.issues.forEach((issue) => {
            const fieldPath = issue.path.join(
              "."
            ) as keyof ArInvoiceCtmDtSchemaType
...
InvoiceCtmDetailsForm.displayName = "InvoiceCtmDetailsForm"

export default InvoiceCtmDetailsForm
