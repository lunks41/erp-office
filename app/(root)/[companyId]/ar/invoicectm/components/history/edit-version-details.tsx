...
import {
  useEffect,
  useState } from "react"
import { IArInvoiceCtmHd } from "@/interfaces"
...
export default function EditVersionDetails({
  ...
  const [selectedInvoice, setSelectedInvoice] =
    useState<IArInvoiceCtmHd | null>(null)
  ...
  const { data: invoiceHistoryData, refetch: refetchHistory } =
    //useGetARInvoiceHistoryList<IArInvoiceCtmHd[]>("14120250100024")
    useGetARInvoiceHistoryList<IArInvoiceCtmHd[]>(invoiceId)

  const { data: invoiceDetailsData, refetch: refetchDetails } =
    useGetARInvoiceHistoryDetails<IArInvoiceCtmHd>(
      selectedInvoice?.invoiceId || "",
      selectedInvoice?.editVersion?.toString() || ""
    )

  function isIArInvoiceCtmHdArray(arr: unknown): arr is IArInvoiceCtmHd[] {
    return (
      Array.isArray(arr) &&
      (arr.length === 0 ||
  ...
  const tableData: IArInvoiceCtmHd[] =
    invoiceHistoryData?.result === 1 &&
    isIArInvoiceCtmHdArray(invoiceHistoryData?.data)
      ? invoiceHistoryData.data
      : []

  // Check if details data is successful and has valid data
  const dialogData: IArInvoiceCtmHd | undefined =
    ...
    !Array.isArray(invoiceDetailsData.data)
      ? (invoiceDetailsData.data as IArInvoiceCtmHd)
      : undefined
  ...
  const columns: ColumnDef<IArInvoiceCtmHd>[] = [
    {
