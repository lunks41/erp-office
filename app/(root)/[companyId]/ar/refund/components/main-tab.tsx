...
export default function Main({
  ...
  const [dataDetails, setDataDetails] = useState<ArRefundDtSchemaType[]>([])
  ...
  const totalBalanceAmt = useMemo(() => {
    return dataDetails.reduce((sum, detail) => {
      const balAmt = Number((detail as unknown as IArRefundDt).docBalAmt) || 0
      return sum + balAmt
    }, 0)
  }, [dataDetails])
  ...
  const removeRefundDetails = useCallback(
    (itemNos: number[]) => {
      ...
      const resetArr = resetData as unknown as IArRefundDt[]
      ...
      const finalResetData: ArRefundDtSchemaType[] = resetData.map(
        (item, index) => ({
          ...item,
  ...
  const handleDataReorder = (newData: IArRefundDt[]) => {
    form.setValue("data_details", newData as unknown as ArRefundDtSchemaType[])
    setDataDetails(newData as unknown as ArRefundDtSchemaType[])
  }

  // ==================== HELPER FUNCTIONS ====================

  const validateAllocation = useCallback((data: ArRefundDtSchemaType[]) => {
    if (!validateAllocationHelper(data as unknown as IArRefundDt[])) {
      return false
    }
    return true
  }, [])
  ...
  const updateAllocationCalculations = useCallback(
    (
      updatedData: ArRefundDtSchemaType[],
      rowIndex: number,
      allocValue: number
    ): number | undefined => {
      const arr = updatedData as unknown as IArRefundDt[]
  ...
  const handleAutoAllocation = useCallback(() => {
    ...
    const result = autoAllocateAmounts(
      currentData as unknown as IArRefundDt[],
      totAmt,
      dec
    )
    const updatedData =
      result.updatedDetails as unknown as ArRefundDtSchemaType[]
  ...
  const handleAddSelectedTransactions = useCallback(
    (transactions: IArOutTransaction[]) => {
      ...
      const newDetails: ArRefundDtSchemaType[] = transactions.map(
        (transaction, index) => ({
          companyId: companyId,
  ...
  return (
    <div className="w-full">
      ...
      <div className="px-2 pt-1">
        ...
        <RefundDetailsTable
          key={refreshKey}
          data={(dataDetails as unknown as IArRefundDt[]) || []}
          visible={visible}
          onDeleteAction={handleDelete}
          onBulkDeleteAction={handleBulkDelete}
          onDataReorder={handleDataReorder as (newData: IArRefundDt[]) => void}
