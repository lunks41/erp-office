...
export default function PaymentDetails({ refundId }: PaymentDetailsProps) {
  ...
  const effectiveRefundId =
    refundId != null &&
    String(refundId).trim() !== "" &&
    String(refundId) !== "undefined"
      ? String(refundId).trim()
      : ""
  ...
  const { data: paymentDetails, refetch: refetchPayment } =
    useGetPaymentDetails<IPaymentHistoryDetails>(
      ...
      effectiveRefundId || "0",
      { enabled: !!effectiveRefundId && effectiveRefundId !== "0" }
    )
