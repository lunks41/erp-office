...
const CreditNoteDetailsForm = React.forwardRef<
...
>(
  (
    ...
    const createDefaultValues = (itemNo: number): ArCreditNoteDtSchemaType => {
      // Use defaults if available, otherwise use defaultCreditNoteDetails values
      const glId =
        defaultGlId && defaultGlId > 0
          ? defaultGlId
          : defaultCreditNoteDetails.glId
      const uomId =
        defaultUomId && defaultUomId > 0
          ? defaultUomId
          : defaultCreditNoteDetails.uomId
      const gstId =
        defaultGstId && defaultGstId > 0
          ? defaultGstId
          : defaultCreditNoteDetails.gstId
    ...
    const populateCodeNameFields = (
      formData: ArCreditNoteDtSchemaType
    ): ArCreditNoteDtSchemaType => {
    ...
    // Uses reusable helper function from ar-creditNote-calculations
    ...
    const onSubmit = async (_data: ArCreditNoteDtSchemaType) => {
      try {
        ...
        if (!validationResult.success) {
          ...
          validationResult.error.issues.forEach((issue) => {
            const fieldPath = issue.path.join(
              "."
            ) as keyof ArCreditNoteDtSchemaType
        ...
        const rowData: IArCreditNoteDt = {
          creditNoteId: updatedData.creditNoteId ?? "0",
          creditNoteNo: updatedData.creditNoteNo ?? "",
          ...
          suppCreditNoteNo: updatedData.suppCreditNoteNo ?? "",
          apCreditNoteId: updatedData.apCreditNoteId ?? "0",
          apCreditNoteNo: updatedData.apCreditNoteNo ?? "",
          ...
          >
            ...
            <div className="col-span-8 mb-1">
              <div className="flex items-center gap-3">
                ...
                >
                  {isCancelled
                    ? "Details (Disabled - CreditNote Cancelled)"
                    : editingDetail
...
CreditNoteDetailsForm.displayName = "CreditNoteDetailsForm"

export default CreditNoteDetailsForm
