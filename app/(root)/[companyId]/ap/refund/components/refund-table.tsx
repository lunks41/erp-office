...
export interface RefundTableProps {
  onRefundSelect: (selectedRefund: IApRefundHd | undefined) => void
...
export default function RefundTable({
  ...
  } = useGetWithDatesAndPagination<IApRefundHd>(
    `${ApRefund.get}`,
    TableName.apRefund,
    searchQuery,
    searchStartDate ?? "",
  ...
  const columns: ColumnDef<IApRefundHd>[] = [
    ...
    },
    {
      ...
      minSize: 100,
      cell: ({ row }) => {
        const raw = row.original as IApRefundHd & { IsCancel?: boolean }
        const isCancel =
