...
>(
  (
    ...
    // Uses reusable helper function from ap-invoice-calculations
    ...
    const onSubmit = async (_data: ApInvoiceDtSchemaType) => {
      try {
        ...
        if (!validationResult.success) {
          ...
          validationResult.error.issues.forEach((issue) => {
            const fieldPath = issue.path.join(
              "."
            ) as keyof ApInvoiceDtSchemaType
        ...
        const rowData: IApInvoiceDt = {
          ...
          custInvoiceNo: updatedData.custInvoiceNo ?? "",
          arInvoiceId: updatedData.arInvoiceId ?? "0",
          arInvoiceNo: updatedData.arInvoiceNo ?? "",
    ...
    const handleChartOfAccountChange = (
      ...
      if (selectedOption) {
        ...
        setIsJobSpecific(selectedOption.isJobSpecific)
