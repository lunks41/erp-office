...
import { recalculateAndSetHeaderTotals } from "@/helpers/ap-adjustment-calculations"
...
export default function Main({
  ...
  const handleDetailsFormRef = (instance: AdjustmentDetailsFormRef | null) => {
  ...
  const recalculateHeaderTotals = () => {
    ...
    recalculateAndSetHeaderTotals(
      form,
      currentDetails as unknown as IApAdjustmentDt[],
      decimals[0],
      visible
    )
  ...
  const handleClone = (detail: IApAdjustmentDt) => {
    const currentData =
      (form.getValues("data_details") as unknown as IApAdjustmentDt[]) || []
    ...
    const clonedDetail: IApAdjustmentDt = {
      ...detail,
      itemNo: nextItemNo,
      seqNo: nextItemNo,
    ...
    setEditingDetail(clonedDetail as unknown as ApAdjustmentDtSchemaType)
    setTableKey((prev) => prev + 1)
  }
