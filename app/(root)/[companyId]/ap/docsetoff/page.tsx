...
export default function DocSetOffPage() {
    ...
    try {
      ...
      if (!validationResult.success) {
        ...
        const fieldLabelMap: Record<string, string> = {
          ...
          supplierId: "Supplier",
          balTotAmt: "Balance Total Amount",
        ...
        validationResult.error.issues.forEach((error) => {
          ...
          const fieldPath = pathKey as keyof ApDocSetOffHdSchemaType
        ...
        if (response.result === 1) {
          ...
          if (docSetOffData) {
            const updatedSchemaType = transformToSchemaType(
              docSetOffData as unknown as IApDocSetOffHd
            )
  ...
  const handleCloneDocSetOff = async () => {
    if (docSetOff) {
      ...
      const clonedDocSetOff: ApDocSetOffHdSchemaType = {
        ...docSetOff,
        setoffId: "0",
        setoffNo: "",
  ...
  const handlePrintDocSetOff = () => {
    ...
    const reportData = {
      reportFile: "ap/ApDocSetOff.trdp",
    ...
    async ({
      ...
      try {
        const response = await getById(
          `${ApDocSetOff.getByIdNo}/${requestDocSetOffId}/${requestDocSetOffNo}`
        )
