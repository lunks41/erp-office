...
>(
  (
    ...
    // Uses reusable helper function from ap-creditNote-calculations
    ...
    const onSubmit = async (_data: ApCreditNoteDtSchemaType) => {
      try {
        ...
        if (!validationResult.success) {
          ...
          validationResult.error.issues.forEach((issue) => {
            const fieldPath = issue.path.join(
              "."
            ) as keyof ApCreditNoteDtSchemaType
        ...
        const rowData: IApCreditNoteDt = {
          ...
          custCreditNoteNo: updatedData.custCreditNoteNo ?? "",
          arCreditNoteId: updatedData.arCreditNoteId ?? "0",
          arCreditNoteNo: updatedData.arCreditNoteNo ?? "",
