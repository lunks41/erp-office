...
export default function Main({
  ...
  const [editingDetail, setEditingDetail] =
    useState<ApCreditNoteDtSchemaType | null>(null)
  ...
  const handleDetailsFormRef = (instance: CreditNoteDetailsFormRef | null) => {
  ...
  const recalculateHeaderTotals = () => {
    ...
    recalculateAndSetHeaderTotals(
      form,
      currentDetails as unknown as IApCreditNoteDt[],
      decimals[0],
      visible
    )
  ...
  const handleAddRow = (rowData: IApCreditNoteDt) => {
  ...
  const handleEdit = (detail: IApCreditNoteDt) => {
    // Convert IApCreditNoteDt to ApCreditNoteDtSchemaType and set for editing
    setEditingDetail(detail as unknown as ApCreditNoteDtSchemaType)
  }
