...
interface CargoTypeFormProps {
  initialData?: ICargoType | null
  submitAction: (data: CargoTypeSchemaType) => void
  onCancelAction?: () => void
...
export function CargoTypeForm({
  ...
  const form = useForm<CargoTypeSchemaType>({
    ...
    defaultValues: initialData
      ? {
          ...
          cargoTypeCode: initialData.cargoTypeCode ?? "",
  ...
  const handleCodeBlur = (_e: React.FocusEvent<HTMLInputElement>) => {
    const code = form.getValues("cargoTypeCode")
    if (code) {
      onCodeBlur?.(code)
    }
  }

  const onSubmit = (data: CargoTypeSchemaType) => {
    submitAction(data)
  }
