...
interface CargoTypesTableProps {
  data: ICargoType[]
  isLoading?: boolean
  totalRecords?: number
  onSelect?: (cargoType: ICargoType | null) => void
  onDeleteAction?: (cargoTypeId: string) => void
  onEditAction?: (cargoType: ICargoType) => void
  onCreateAction?: () => void
...
export function CargoTypesTable({
  ...
  canView = true,
  canCreate = true,
}: CargoTypesTableProps) {
  const { decimals } = useAuthStore()
  const datetimeFormat = decimals[0]?.longDateFormat || "dd/MM/yyyy HH:mm:ss"

  const columns: ColumnDef<ICargoType>[] = [
    {
      accessorKey: "cargoTypeCode",
      header: "Code",
      size: 120,
      minSize: 50,
