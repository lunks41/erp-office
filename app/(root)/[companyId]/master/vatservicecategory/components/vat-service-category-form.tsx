...
const defaultValues = {
  ...
  vatServiceCategoryName: "",
  vatServiceCategoryCode: "",
  serviceCategoryId: 0,
  seqNo: 0,
  remarks: "",
  isActive: true,
}
interface VATServiceCategoryFormProps {
  initialData?: IVATServiceCategory | null
  submitAction: (data: VATServiceCategorySchemaType) => void
  onCancelAction?: () => void
...
export function VATServiceCategoryForm({
  initialData,
  submitAction,
  onCancelAction,
  isSubmitting = false,
  isReadOnly = false,
  onCodeBlur,
}: VATServiceCategoryFormProps) {
  const { decimals } = useCompanyStore()
  const datetimeFormat = decimals[0]?.longDateFormat || "dd/MM/yyyy HH:mm:ss"

  const form = useForm<VATServiceCategorySchemaType>({
  ...
  const onSubmit = (data: VATServiceCategorySchemaType) => {
    submitAction(data)
  }
