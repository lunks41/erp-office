...
interface TaxCategoryTableProps {
  ...
  isLoading?: boolean
  totalRecords?: number
  onSelect?: (taxdtcategory: ITaxCategory | null) => void
  onDeleteAction?: (taxId: string) => void
  onEditAction?: (taxdtcategory: ITaxCategory) => void
  onCreateAction?: () => void
...
export function TaxCategoryTable({
  data,
  isLoading = false,
  totalRecords = 0,
  onSelect,
  ...
  canEdit = true,
  canDelete = true,
}: TaxCategoryTableProps) {
  const { decimals } = useAuthStore()
  const datetimeFormat = decimals[0]?.longDateFormat || "dd/MM/yyyy HH:mm:ss"

  const columns: ColumnDef<ITaxCategory>[] = [
    {
      accessorKey: "taxCategoryCode",
      header: "Code",
      cell: ({ row }) => (
        <div className="font-medium">{row.getValue("taxCategoryCode")}</div>
      ),
      size: 120,
      minSize: 50,
