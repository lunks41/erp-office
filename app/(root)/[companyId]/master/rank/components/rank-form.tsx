...
import { RankSchemaType, rankSchema } from "@/schemas/rank"
...
interface RankFormProps {
  initialData?: IRank | null
  submitAction: (data: RankSchemaType) => void
  onCancelAction?: () => void
...
export function RankForm({
  ...
  const form = useForm<RankSchemaType>({
  ...
  const handleCodeBlur = (_e: React.FocusEvent<HTMLInputElement>) => {
    const code = form.getValues("rankCode")
    if (code) {
      onCodeBlur?.(code)
    }
  }

  const onSubmit = (data: RankSchemaType) => {
    submitAction(data)
  }
