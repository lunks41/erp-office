...
import { VisaSchemaType, visaSchema } from "@/schemas/visa"
...
interface VisaFormProps {
  initialData?: IVisa | null
  submitAction: (data: VisaSchemaType) => void
  onCancelAction?: () => void
...
export function VisaForm({
  ...
  const form = useForm<VisaSchemaType>({
  ...
  const handleCodeBlur = (_e: React.FocusEvent<HTMLInputElement>) => {
    const code = form.getValues("visaCode")
    if (code) {
      onCodeBlur?.(code)
    }
  }

  const onSubmit = (data: VisaSchemaType) => {
    submitAction(data)
  }
