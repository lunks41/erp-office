...
export type CbPettyCashHdSchemaOptions = {
  chequePaymentTypeIds?: number[]
}

export const CbPettyCashHdSchema = (
  required: IMandatoryFields,
  visible: IVisibleFields,
  options?: CbPettyCashHdSchemaOptions
) => {
  ...
  return z.object({
    ...
    // Nested Details
    data_details: z
      .array(CbPettyCashDtSchema(required, visible))
...
export type CbPettyCashHdSchemaType = z.infer<
  ReturnType<typeof CbPettyCashHdSchema>
>

export const CbPettyCashHdFiltersSchema = z.object({
  isActive: z.boolean().optional(),
  search: z.string().optional(),
  sortOrder: z.enum(["asc", "desc"]).optional(),
})

export type CbPettyCashHdFiltersValues = z.infer<
  typeof CbPettyCashHdFiltersSchema
>

export const CbPettyCashDtSchema = (
  required: IMandatoryFields,
  visible: IVisibleFields
) => {
  return z
    ...
    .refine(
      (data) => {
        // If GST percentage is above 0%, supplierRegNo (TRN No) is required
        const gstPercentage = data.gstPercentage ?? 0
        if (gstPercentage > 0) {
          return data.supplierRegNo && data.supplierRegNo.trim().length > 0
        }
        return true
      },
      {
...
export type CbPettyCashDtSchemaType = z.infer<
  ReturnType<typeof CbPettyCashDtSchema>
>

export const CbPettyCashDtFiltersSchema = z.object({
  isActive: z.boolean().optional(),
  search: z.string().optional(),
  sortOrder: z.enum(["asc", "desc"]).optional(),
})

export type CbPettyCashDtFiltersValues = z.infer<
  typeof CbPettyCashDtFiltersSchema
>
